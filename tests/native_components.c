...
int main(void) {
  ...
  const char* input_arguments[] = {
    ...
    "Write-Output 'ready'; $line = [Console]::ReadLine(); Write-Output ('input:' + $line); Write-Output ('size:' + $Host.UI.RawUI.WindowSize.Width + 'x' + $Host.UI.RawUI.WindowSize.Height); exit 0",
    NULL
  };
#else
